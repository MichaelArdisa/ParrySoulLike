...
{
    public static class SubMaterialCombiner
    {
        public static void CombineSubMaterials(SkinnedMeshRenderer smr, string path, int aniso, int maxTex)
        {
            ...
            string matPath = $"{path}/SubMaterials/{smr.name}_Combined_Sub.mat";
            ...
            string meshPath = $"{path}/SubMaterials/{smr.name}_CombinedMesh.asset";
        ...
        private static Dictionary<string, List<Texture2D>> InitTexDict()
        {
            ...
            {
                { "_MainTex", new List<Texture2D>() },
                { "_BumpMap", new List<Texture2D>() },
                { "_MetallicGlossMap", new List<Texture2D>() },
        ...
        private static Texture2D GetTex(Material mat, string texName, List<Texture2D> texList)
        {
            ...
            if (tex != null)
            {
                EnsureReadable(tex, texName == "_BumpMap");
                texList.Add(tex);
                return tex;
            }
        ...
        private static Dictionary<string, Texture2D> SaveAtlases(string name, Dictionary<string, List<Texture2D>> texDict, string path, int aniso, int maxTex, out Dictionary<string, Rect[]> rects)
        {
            ...
            foreach (var kvp in texDict)
            {
                ...
                SaveAtlas(atlas, atlasPath, kvp.Key == "_BumpMap", aniso, maxTex);
        ...
        private static Texture2D NewAtlas(Texture2D nm)
        {
            int w = nm.width;
            int h = nm.height;
            Texture2D adj = new Texture2D(w, h, TextureFormat.RGBA32, false);

            Color[] px = nm.GetPixels();
            for (int y = 0; y < h; y++)
            {
                for (int x = 0; x < w; x++)
                {
                    Color c = px[y * w + x];
                    float r = Mathf.Sqrt(1.0f - Mathf.Pow(c.g * 2 - 1, 2) - Mathf.Pow(c.b * 2 - 1, 2));
                    float g = Mathf.Clamp01((c.g * 2 - 1) * 0.5f + 0.5f);
                    float b = Mathf.Clamp01((c.b * 2 - 1) * 0.5f + 0.5f);
                    adj.SetPixel(x, y, new Color(g, b, r, c.a));
                }
            }
        ...
        private static Mesh CombineMesh(Mesh originalMesh, Material[] mats, Dictionary<string, Rect[]> rects)
        {
            ...
            for (int subMeshIdx = 0; subMeshIdx < originalMesh.subMeshCount; subMeshIdx++)
            {
                ...
                if (!rects.ContainsKey("_MainTex") || subMeshIdx >= rects["_MainTex"].Length)
                {
                    Debug.LogError($"Missing UV rect for subMeshIdx {subMeshIdx} in _MainTex.");
                    continue;
                }

                var uvRect = rects["_MainTex"][subMeshIdx];
