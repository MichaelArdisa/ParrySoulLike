...
{
    public static class URPMaterialCombiner
    {
        public static void CombineSubMaterials(SkinnedMeshRenderer smr, string path, int aniso, int maxTex)
        {
            ...
            string matPath = $"{path}/SubMaterials/{smr.name}_Combined_Sub_URP.mat";
            ...
            string meshPath = $"{path}/SubMaterials/{smr.name}_CombinedMesh_URP.asset";
        ...
        private static Mesh CombineMesh(Mesh originalMesh, Material[] mats, Dictionary<string, Rect[]> rects)
        {
            ...
            for (int subMeshIdx = 0; subMeshIdx < originalMesh.subMeshCount; subMeshIdx++)
            {
                ...
                if (!rects.ContainsKey("_BaseMap") || subMeshIdx >= rects["_BaseMap"].Length)
                {
                    Debug.LogError($"Missing UV rect for subMeshIdx {subMeshIdx} in _BaseMap.");
                    continue;
                }

                var uvRect = rects["_BaseMap"][subMeshIdx];
